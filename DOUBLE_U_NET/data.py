...
def augment_data(images, masks, save_path, augment=True):
    ...
    for image, mask in tqdm(zip(images, masks), total=len(images)):
        ...
        if augment == True:
            ...
            aug = MedianBlur(p=1, blur_limit=3)
            augmented = aug(image=x, mask=y)
            x21 = augmented['image']
            y21 = augmented['mask']
...
def get_skin_lesion_data(path, split=0.1):
    ...
    memasses_x = glob(os.path.join(path, "memassesx/*"))
    memasses_y = glob(os.path.join(path, "memassesy/*"))

    return (train_x, train_y), (valid_x, valid_y), (test_x, test_y), (memasses_x, memasses_y)

def main():
    np.random.seed(42)
    path = "./"
    (train_x, train_y), (valid_x, valid_y), (test_x, test_y), (memasses_x, memasses_y) = get_skin_lesion_data(path, split=0.1)
    ...
    # create_dir("new_data/memasses/image/")
    # create_dir("new_data/memasses/mask/")

    # augment_data(train_x, train_y, "new_data/train/", augment=True)
    # augment_data(valid_x, valid_y, "new_data/valid/", augment=False)
    # augment_data(test_x, test_y, "new_data/test/", augment=False)
    augment_data(memasses_x, memasses_y, "new_data/memasses/", augment=True)
